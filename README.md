...
I first came across Stefan Abi-Karam's excellent FPGA placement project[^2] and was immediately hooked. It sits at the intersection of three things I find fascinating: FPGAs (programmable hardware that feels like magic), combinatorial optimisation (the art of finding needles in exponentially large haystacks), and Rust (because why make life easy?). Stefan's article is genuinely excellent technical writing - it introduces the placement problem clearly, discusses the trade-offs between different algorithmic approaches, and provides clean, working Rust code. I highly recommend reading it first if you haven't already.

After working through Stefan's implementation, I started thinking about extensions. What would happen if I implemented true simulated annealing with proper temperature schedules and the Metropolis acceptance criterion? And what about testing on real circuit benchmarks rather than the synthetic random graphs Stefan used? Stefan makes an interesting observation in his article: _"simulated annealing (SA) is a misnomer as you don't really need the 'annealing' part to make it work."_ This seemed like something I could test empirically. This post documents my journey extending Stefan's original project - implementing true SA, adding real benchmark support, and running the experiments to see what the data actually shows.
...
**Placement** is the step where we decide _where_ on the chip each circuit element goes. Good placement makes the subsequent routing step easier and results in faster, more power-efficient designs.
...
Stefan's article provides valuable context on the broader landscape of placement algorithms. He discusses three main approaches:
...
For his implementation, Stefan takes a pragmatic approach. Rather than implementing classical SA with temperature schedules and probabilistic acceptance, he uses what I will call **greedy multi-neighbour descent**:
...
Stefan's implementation includes an important domain-specific optimisation: the **directed move**. Rather than moving a node to a random valid location, this action moves a node toward the **centroid** of all placed nodes:
...
Stefan observed an interesting phenomenon: increasing `n_neighbors` helps up to a point, then performance degrades. My experiments confirm this:
...
In 1983, Kirkpatrick, Gelatt, and Vecchi realised this physical process could be simulated computationally to solve combinatorial optimisation problems[^1]. Their seminal paper showed how the **Metropolis algorithm** from statistical physics could be adapted for optimisation.
...
To understand why SA might outperform greedy descent, we need to understand the **cost landscape**:
...
Building on Stefan's foundation, I implemented the machinery needed to answer this question empirically.
...
In a Manhattan routing structure (where wires can only travel horizontally or vertically), the HPWL of a net approximates the length of a [Steiner tree](https://en.wikipedia.org/wiki/Steiner_tree_problem) - the minimum-length tree connecting all terminals. This makes HPWL a tight lower bound on the actual routing cost.
...
See [Analytical Minimization of Half-Perimeter Wirelength (ASP-DAC 2000)](https://dl.acm.org/doi/10.1145/368434.368600) and [Half-Perimeter Wirelength Model for VLSI Analytical Placement (IEEE 2014)](https://ieeexplore.ieee.org/document/7033338/) for detailed analysis of these approximations.
...
Stefan's original implementation uses **Erdos-Renyi random graphs** to generate netlists.
...
I extended the comparison to real circuit benchmarks from the GSRC Bookshelf[^4] to validate the findings on industry-standard test cases.
...
I implemented a parser for these files to load real circuit benchmarks:
